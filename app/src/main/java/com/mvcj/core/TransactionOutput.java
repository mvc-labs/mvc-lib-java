...
public class TransactionOutput extends ChildMessage {
    ...
    // These fields are not mvc serialized. They are used for tracking purposes in our wallet
    ...
    /**
     * <p>If the output script pays to an address as in <a href="https://mvc.org/en/developer-guide#term-p2pkh">
     ...
     * the redeem script.</p>
     *
     * <p>P2SH is described by <a href="https://github.com/mvc/bips/blob/master/bip-0016.mediawiki">BIP 16</a> and
     * <a href="https://mvc.org/en/developer-guide#p2sh-scripts">documented in the mvc Developer Guide</a>.</p>
     *
     * @param networkParameters needed to specify an address
    ...
    /**
     * <p>Gets the minimum value for a txout of this size to be considered non-dust by mvc Core
     * (and thus relayed). See: CTxOut::IsDust() in mvc Core. The assumption is that any output that would
     * consume more than a third of its value in fees is not something the mvc system wants to deal with right now,
     ...
     *
     * @param feePerKb The fee required per kilobyte. Note that this is the same as mvc Core's -minrelaytxfee * 3
     */
    public Coin getMinNonDustValue(Coin feePerKb) {
        ...
        // formula is wrong for anything that's not a pay-to-address output, unfortunately, we must follow mvc Core's
