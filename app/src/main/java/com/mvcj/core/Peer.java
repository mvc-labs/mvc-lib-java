...
import com.showpay.showmoneywalletsdk.org.mvcj.net.StreamConnection;
...
import com.showpay.showmoneywalletsdk.org.mvcj.net.AbstractTimeoutHandler;
import com.showpay.showmoneywalletsdk.org.mvcj.net.NioClient;
...
/**
 * <p>A Peer handles the high level communication with a mvc node, extending a {@link PeerSocketHandler} which
...
public class Peer extends PeerSocketHandler {
        ...
        try {
            ...
            try {
                if (awaitingFreshFilter != null) {
                    ...
                    // range of blocks and get an inv both times, due to the codepath in mvc Core hitting
            ...
            if (blocks.size() > 0 && downloadData && blockChain != null) {
                ...
                for (InventoryItem item : blocks) {
                    if (blockChain.isOrphan(item.hash) && downloadBlockBodies) {
                        ...
                        //
                        // Note that as of June 2012 mvc Core won't actually ever interleave blocks pushed as
                        // part of chain download with newly announced blocks, so it should always be taken care of by
                        // the duplicate check in blockChainDownloadLocked(). But mvc Core may change in future so
    ...
    private void blockChainDownloadLocked(Sha256Hash toHash) {
        ...
        //
        //   https://en.mvc.it/wiki/Protocol_specification#getblocks
        //
     ...
     * after the query returns. <b>Most peers do not support this request. You will need to connect to mvc XT
     ...
     * <b>Most peers do not support this request. You will need to connect to mvc XT peers if you want
     * this to work.</b>
     *
    ...
    public ListenableFuture<UTXOsMessage> getUTXOs(List<TransactionOutPoint> outPoints, boolean includeMempool) {
        ...
        try {
            ...
            if ((peerVer.localServices & GetUTXOsMessage.SERVICE_FLAGS_REQUIRED) != GetUTXOsMessage.SERVICE_FLAGS_REQUIRED)
                throw new ProtocolException("Peer does not support getutxos protocol flag: find mvc XT nodes.");
