...
package com.mvcj.uri;

import com.showpay.showmoneywalletsdk.org.mvcj.core.Address;
import com.showpay.showmoneywalletsdk.org.mvcj.core.AddressFormatException;
...
import com.showpay.showmoneywalletsdk.org.mvcj.params.AbstractBitcoinNetParams;
...
/**
 * <p>Provides a standard implementation of a mvc URI with support for the following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code mvc:<address>}</li>
 * <li>{@code mvc:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://github.com/mvc/bips/blob/master/bip-0021.mediawiki">BIP 0021</a>
 */
public class CoinURI {
    ...
    /**
     * URI for mvc network. Use {@link AbstractBitcoinNetParams#BITCOIN_SCHEME} if you specifically
     * need mvc, or use {@link NetworkParameters#getUriScheme} to get the scheme
     ...
     * @throws URIParseException if the URI is not syntactically or semantically valid.
     */
    public CoinURI(String uri) throws URIParseException {
        this(null, uri);
    }

    /**
     * Constructs a new object by trying to parse the input as a valid mvc URI.
     *
     ...
     *
     * @throws URIParseException If the input fails mvc URI syntax and semantic checks.
     */
    public CoinURI(@Nullable NetworkParameters params, String input) throws URIParseException {
        checkNotNull(input);
        ...
        try {
            ...
            throw new URIParseException("Bad URI syntax", e);
        }

        // URI is formed as  mvc:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form mvc://address?....
        // We support both until Ben fixes his code.
        
        // Remove the mvc scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : mvc:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        ...
        if (input.startsWith(blockchainInfoScheme)) {
            ...
            throw new URIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?", 2);
        if (addressSplitTokens.length == 0)
            throw new URIParseException("No data found after the mvc: prefix");
        ...
        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a mvc address for this network
            try {
                Address address = Address.fromBase58(params, addressToken);
                putWithValidation(FIELD_ADDRESS, address);
            } catch (final AddressFormatException e) {
                throw new URIParseException("Bad address", e);
            }
        }

        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new URIParseException("No address and no r= parameter found");
        }
    }
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws URIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new URIParseException("Malformed mvc URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new URIParseException("Malformed mvc URI - empty name '" +
                        nameValuePairToken + "'");
            ...
            if (FIELD_AMOUNT.equals(nameToken)) {
                ...
                try {
                    ...
                    if (params != null && amount.isGreaterThan(params.getMaxMoney()))
                        throw new URIParseException("Max number of coins exceeded");
                    if (amount.signum() < 0)
     ...
     */
    private void putWithValidation(String key, Object value) throws URIParseException {
        if (parameterMap.containsKey(key)) {
            throw new URIParseException(String.format(Locale.US, "'%s' is duplicated, URI is invalid", key));
        } else {
    ...
    /**
     * The mvc Address from the URI, if one was present. It's possible to have mvc URI's with no address if a
    ...
    /**
     * Simple mvc URI builder using known good fields.
     *
     * @param address The mvc address
     * @param amount The amount
     * @param label A label
     * @param message A message
     * @return A String containing the mvc URI
     */
